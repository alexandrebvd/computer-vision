...
DOCUMENT_HEIGHT_WIDTH_RATIO = np.sqrt(2) # ISO paper size concept (A0, A1, A2, A3, ...)
WIDTH = 500
...
def onMouse(event, x, y, flags, param):
    ...
    if event == (cv2.EVENT_LBUTTONDOWN and cv2.EVENT_MOUSEMOVE):
        if drawing:
            ...
            cv2.drawContours(imageCopy, [approx], -1, CONTOUR_COLOR, thickness=2) #Uncomment to draw lines conecting the edges
            cv2.drawContours(imageCopy, approx, -1, EDGE_COLOR, thickness=20) #Uncomment to draw points on edges
...
#imageGray = cv2.bilateralFilter(imageGray, 11, 13, 13)
# Edge detection
